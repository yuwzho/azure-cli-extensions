...
class AppTransformer(Transformer):
    ...
    def _pcf_to_bicep(self, pcf, output, **__):
        for app in pcf.applications:
            ...
            app_resource = BicepResource(app.name, 'Microsoft.AppPlatform/Spring/Apps', '2022-12-01', parent=spring)
            output.append(app_resource)
            deployment = BicepResource(app.name, 'Microsoft.AppPlatform/Spring/Apps/Deployments', '2022-12-01', parent=app_resource)
            deployment.properties['active'] = True
            output.append(deployment)

    def _check_pcf_to_bicep_violation(self, pcf, **__):
        for app in pcf.applications:
            self._check_app_name(app.name)
